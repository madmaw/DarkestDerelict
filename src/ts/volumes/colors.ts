const COLOR_WHITE_GLOWING: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'yyyy' : [255, 255, 255, 255];
const COLOR_WHITE_SHINY: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'yyy=' : [255, 255, 255, 12];
const COLOR_YELLOW_GLOWING: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'yyBy' : [255, 255, 32, 255];
const COLOR_RED_GLOWING: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'yBBy' : [255, 32, 32, 255];
const COLOR_ORANGE_SHINY: Vector4 | string = FLAG_USE_PACKED_COLORS ? '\\QJ=' : [138, 92, 64, 12];
const COLOR_YELLOW_SHINY: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'ZZJ=' : [128, 128, 64, 12];
const COLOR_SILVER_SHINY: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'ZZZ=' : [128, 128, 128, 12];
const COLOR_GOLD: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'ZZQ=' : [128, 128, 92, 12];
const COLOR_DULLMETAL: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'QQW>' : [92, 92, 118, 18];
const COLOR_GUNMETAL: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'WWZ<' : [118, 118, 128, 9];
const COLOR_GREEN_SHINY: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'QZQ=' : [92, 128, 92, 12];
const COLOR_BLUE_SHINY: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'QQZ<' : [92, 92, 128, 9];
const COLOR_RED_CARPET: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'QFGR' : [92, 48, 52, 99];
const COLOR_RED_SHINY: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'QFF=' : [92, 48, 48, 12];
const COLOR_GREEN_CARPET: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'BFBR' : [44, 66, 44, 99];
const COLOR_BLUE_CARPET: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'EEJR' : [44, 44, 66, 99];
const COLOR_LEATHER: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'E@:l' : [44, 24, 0, 200];
const COLOR_BLUE_GLOWING: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'EEyy' : [44, 44, 255, 255];
const COLOR_CYAN_GLOWING: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'Byyy' : [32, 255, 255, 255];
const COLOR_GREEN_GLOWING: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'ByBy' : [32, 255, 32, 255];
const COLOR_CHITIN: Vector4 | string = FLAG_USE_PACKED_COLORS ? 'BBB<' : [32, 32, 32, 9];
const COLOR_BLACK: Vector4 | string = FLAG_USE_PACKED_COLORS ? ':::y' : [0, 0, 0, 255];
